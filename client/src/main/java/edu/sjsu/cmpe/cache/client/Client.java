...
public class Client {
	public static void main(String[] args) throws Exception {
		...
		for (int i = 1; i <= 10; i++)
		{
			...
			System.out.println("Place " + dataValue+ " into server "+ cache.toString());
		...
		for (int i = 1; i <= 10; i++)
		{
			...
			bucket = Hashing.consistentHash(Hashing.md5().hashString(Integer.toString(i)),nodeLookUp.getSize());
			cache = nodeLookUp.get(bucket);
			System.out.println("Server fetches "+ cache.toString() + " : " + cache.get(i));
		}

		System.out.println("Cache Client Exits...");
	}
